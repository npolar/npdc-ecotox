...
module Couch
  ...
  #  auth2 = Couch::Config::AUTH4
   ...
   def self.rightsholder(inp)
     return (inp == "NPI, Örebro" ? "NPI, Örebro University" : inp)
   end
   ...
   'GQ'=> ['B7_1059a',"pesticides and industrial by products"],
    ...
    'XB'=> ['unidentified_MeSO2_Cl6_PCB',"metabolized pesticids and industrial by products"],
   ...
   'AAN' => ['PCB_128',"polychlorinated biphenyls (PCBs)"]  #sum_ar_1260
    ...
    Dir.glob('./leser/*.xls*') do |excel_file|
     ...
     while (line < (s.last_row).to_i + 1) && ((s.cell(line,'C') != nil) || ((s.cell(line,'G')!= nil)))
         ...
         @ecotox_fieldwork_entry = {
          :database_sample_id => uuid_fieldwork + "-" + (line-1).to_s,
          :NPI_sample_id => checkExistence(s.cell(line,'H')).to_s,
          :project_group => s.cell(line,'A'),
          :rightsholder => rightsholder(s.cell(line,'Q')),
          :people_responsible => checkExistence(s.cell(line,'W'))+ " " + checkExistence(s.cell(line,'X'))+ "," +checkExistence(s.cell(line,'Y')),
          ...
          :egg_width => checkExistence(s.cell(line,'AAJ')),
        ...
        header_arr.each do | key, value |
          ...
          if (analyte.length > 0)
            ...
            @lab_ecotox = {
              ...
              :lang => 'no',
              :laboratory => laboratory(checkExistence(s.cell(line,'B'))),
              :lab_report_id => s.cell(line,'XD'),
              :date_report => iso8601time(checkExistence(s.cell(line,'M'))),
              :rightsholder => rightsholder(checkExistence(s.cell(line,'Q'))),
              :people_responsible => checkExistence(s.cell(line,'W'))+ " " + checkExistence(s.cell(line,'X'))+ "," + checkExistence(s.cell(line,'Y')),
              :matrix => matrix(checkExistence(s.cell(line,'G'))),
              :species => s.cell(line,'C'),
              :database_sample_id => uuid_fieldwork + "-" + (line-1).to_s,
              :NPI_sample_id => checkExistence(s.cell(line,'H')).to_s,
              :lab_sample_id => s.cell(line,'J'),
              :fat_percentage => s.cell(line,'K').to_f,
              :unit => unit(checkExistence(s.cell(line,'AB'))),
              :comment => checkExistence(s.cell(line,'V')) + " " + checkExistence(s.cell(line,'Z')) + teq,
              :analyte_category => "#{value[1]}",
              :analyte => "#{value[0]}",
              :wet_weight => analyte_arr[0],
            ...
            #post entry
            postToServer('https://' + host + '/lab/ecotox/'+uuid_ecotox,doc_ecotox,auth,user,password)
          end #if
          ...
          biomarker_arr.each do | key2, value2 |
            ...
            #puts doc_biomarker
            #post entry
            postToServer('https://' + host + '/lab/biomarker/'+uuid_biomarker,doc_biomarker,auth,user,password)
          end #if
          end #traverse analytes
    ...
    #post entry
    postToServer('https://' + host + '/ecotox/fieldwork/'+uuid_fieldwork,doc_fieldwork,auth,user,password)
